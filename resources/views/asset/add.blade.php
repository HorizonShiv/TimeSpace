                ...
                @foreach ($CampaignData->Flight as $Flights)
                    ...
                    <div id="Flight_{{ $Flights->flight_count }}" {{ $Flights->flight_count != 1 ? 'hidden' : '' }}>
                        ...
                        @foreach ($FlightConnectionData as $flightsConnections)
                            @foreach ($flightsConnections as $Campaign)
                                @if ($Campaign->language === 'EN')
                                    ...
                                    <div class="flex w-full flex-col gap-4 my-4 {{ $multiAssetsCounter != 0 ? 'hidden' : '' }}"
                                        id="CopyContainer_{{ $Campaign->tag }}_{{ $Campaign->CategoryMaster->id }}">
                                        ...
                                        <div class="flex w-full flex-col">
                                            <div class="flex justify-between items-center">
                                                ...
                                                <div class="flex gap-2">
                                                    ...
                                                    @foreach ($tags[$Flights->id][$Campaign->language][$Campaign->type][$Campaign->tag] as $Category)
                                                        @php
                                                            $categoryData = explode('_', $Category);
                                                        @endphp
                                                        <span id="Span_{{ $Campaign->tag }}_{{ $categoryData[1] }}"
                                                            style="cursor: pointer;"
                                                            onclick="categoryShow('{{ $Campaign->tag }}',{{ $categoryData[1] }},{{ $Campaign->id + $counterAdder }})"
                                                    ...
                                                    @php
                                                        $CategoryName =
                                                            $html[$Flights->id][$Campaign->id][$Campaign->language][
                                                                $Campaign->type
                                                            ][$Campaign->CategoryMaster->id]['categoryName'];
                                            ...
                                            <div class="flex w-full flex-col gap-2 relative" id=""
                                                ...
                                                <div class="w-full flex justify-between">
                                                    <div class="w-auto flex gap-3" {{-- id="inputFieldsNameContainer_{{ $Campaign->id }}" --}}>
                                                        {{-- Labels of all the fields --}}
                                                        {!! $html[$Flights->id][$Campaign->id][$Campaign->language][$Campaign->type][$Campaign->CategoryMaster->id][
                                                            'htmlFieldName'
                                                        ] !!}
                                                        <span class="w-auto flex gap-3"
                                                            id="inputFieldsNameContainer_{{ $Flights->id }}_{{ $Campaign->id }}_{{ $Campaign->language }}_{{ $Campaign->type }}_{{ $Campaign->CategoryMaster->id }}_{{ $ContainerCounter }}"></span>
                                                    </div>
                                                    <div class="z-10 mr-3">
                                                ...
                                                <div class="w-full flex justify-between">
                                                    <div class="w-full flex gap-3" {{-- id="inputFieldsContainer_{{ $Campaign->id }}" --}}>
                                                        {{-- all the input fields --}}
                                                        {!! $html[$Flights->id][$Campaign->id][$Campaign->language][$Campaign->type][$Campaign->CategoryMaster->id][
                                                            'htmlField'
                                                        ] !!}
                                                        <span class="w-auto flex gap-3"
                                                            id="inputFieldsContainer_{{ $Flights->id }}_{{ $Campaign->id }}_{{ $Campaign->language }}_{{ $Campaign->type }}_{{ $Campaign->CategoryMaster->id }}_{{ $ContainerCounter }}"></span>
                                                    </div>
                                                    <div class="z-10 mr-3">
                                                        {!! $html[$Flights->id][$Campaign->id][$Campaign->language][$Campaign->type][$Campaign->CategoryMaster->id][
                                                            'htmlData'
                                            ...
                                            <!-- End of Row Template -->
                                            <div id="AddContainer_{{ $Campaign->id }}"></div>
                                        </div>
                                    ...
                                    <div class="{{ $multiAssetsCounter != 0 ? 'hidden' : '' }}"
                                        id="container_{{ $Campaign->tag }}_{{ $Campaign->CategoryMaster->id }}">
                                        <button type="button" id="addRowBtn"
                                            onclick="AddRow({{ $Flights->id }},{{ $Campaign->id }},'{{ $Campaign->language }}','{{ $Campaign->type }}',{{ $Campaign->CategoryMaster->id }},'{{ $CategoryName }}',{{ $ContainerCounter }})"
                                ...
                                @if ($Campaign->language === 'FR')
                                    ...
                                    @endif
                                    @php
                                        $frenchAssetsDisplayed = true; // Set the variable to true to indicate French assets have been displayed
                                        $counter++;
                                    @endphp
                    ...
                    .then(data => {
                        ...
                        htmlContent += `
                            <div class="w-full flex gap-3" ="">
                                <select
                                required
                                    id="publisher_` + CategoryMasterId + `_` + counter + `"
                                    onchange="getPublisherAdType(` + CategoryMasterId + `,` + counter + `)"
                                    class="select select-primary w-32 rounded-md"
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    $.each(response, function(key, value) {
                        $('#AdType_' + counterId).append('<option value="' + value.id +
                            '">' + value.name + '</option>');
                    });
                },
            });
