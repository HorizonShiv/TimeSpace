...
@section('main-container')
    <form action="{{ route('assets-update') }}" method="POST">
        ...
        <div class="ml-0 md:ml-[300px] mt-[55px] py-7 px-10 w-full" id="content">
            <div>
                ...
                <div class="my-7">
                    <div class="flex flex-col w-full gap-7">
                        <div class="flex flex-col gap-1">
                            <p class="text-xl font-semibold">{{ $Campaign->team->name ?? '' }}</p>
                        ...
                        <div class="join">
                            @foreach ($CampaignData->Flight as $Campaign)
                                <input class="join-item btn border-accent bg-white px-10 hover:bg-white hover:border-accent"
                                    ...
                                    aria-label="Flight {{ $Campaign->flight_count }}" />
                            @endforeach
                        </div>
                    </div>
                    <div class="">
                        <a href="{{ route('flight-edit', $CampaignData->id) }}">
                            <button type="button"
                                class="btn btn-outline border border-primary rounded-full hover:bg-transparent hover:border hover:border-primary">
                                <img src="/assets/sidebar/settings.png" class="w-[20px] h-[20px]" />
                                <p class="text-sm font-medium text-[#555555]">
                ...
                @foreach ($CampaignData->Flight as $Flights)
                    ...
                    <div id="Flight_{{ $Flights->flight_count }}" {{ $Flights->flight_count != 1 ? 'hidden' : '' }}>
                        ...
                        @foreach ($FlightConnectionData as $flightsConnections)
                            @foreach ($flightsConnections as $FlightConnection)
                                @if ($FlightConnection->language === 'EN')
                                    @if (!$englishAssetsDisplayed)
                                        <div class="my-6">
                                            ...
                                            <div class="bg-[#EA33C0] h-[2px] mt-2"></div>
                                        </div>
                                    @endif
                                    @php
                                            ...
                                            <div class="flex justify-between items-center">
                                                ...
                                                <div class="flex gap-2">
                                                    ...
                                                    @foreach ($tags[$Flights->id][$FlightConnection->language][$FlightConnection->type][$FlightConnection->tag] as $Category)
                                                        ...
                                                        @endphp
                                                        <span
                                                            id="Span_{{ $FlightConnection->tag }}_{{ $categoryData[1] }}"
                                                ...
                                                @foreach ($FlightConnection->Assets as $Asset)
                                                    @if ($Asset->flight_id == $Flights->id && $FlightConnection->CategoryMaster->id == $Asset->category_id)
                                                        <div class="flex w-full flex-col gap-2 relative" id=""
                                                            data-row-template>
                                                            <div
                                                                class="absolute bg-[#bababa73] w-[190px] h-full right-0 top-[-8px]">
                                                            </div>
                                        ...
                                        <button type="button" id="addRowBtn"
                                            ...
                                            <svg xmlns="http://www.w3.org/2000/svg" width="15.481" height="15.481"
                                                ...
                                                <path id="add_FILL0_wght400_GRAD0_opsz24"
                                                    ...
                                                    transform="translate(-200 760)" fill="#ea33c0">
                                                </path>
                                            </svg>
                                            Add Row
                                        </button>
                                ...
                                @if ($FlightConnection->language === 'FR')
                                    ...
                                    @endif
                                    @php
                                        $frenchAssetsDisplayed = true; // Set the variable to true to indicate English assets have been displayed
                                        $counter++;
                                    @endphp
                                                        ...
                                                        <div class="flex w-full flex-col gap-2 relative" id=""
                                                            ...
                                                            <div class="w-full flex justify-between">
                                                                <div class="w-auto flex gap-3" {{-- id="inputFieldsNameContainer_{{ $FlightConnection->id }}" --}}>
                                                                    {{-- Labels of all the fields --}}
                                                                    {{-- {{ dd(
                                                                    $html[$Flights->id][$Asset->flight_connection_id]
                                                                ) }} --}}
                                                                    {{-- Labels of all the fields --}}
                                                                    {!! $html[$Flights->id][$Asset->flight_connection_id][$FlightConnection->language][$FlightConnection->type][
                                                                        $FlightConnection->CategoryMaster->id
                ...
                <!-- Save Changes Button -->
                <div class="mt-10">
                    <button class="btn rounded-full bg-accent text-white no-animation w-48">
                        Save Changes
                    </button>
                </div>
            </div>
    ...
    counter++;
    let htmlContent = `
        <div class="flex w-full flex-col gap-2 relative" id="CopyContainer_9" data-row-template="">
            <div class="absolute bg-[#bababa73] w-[190px] h-full right-0 top-[-8px]"></div>
            <div class="w-full flex justify-between">
                <div class="w-auto flex gap-3">
                    <p class="text-base font-normal w-32">Publisher</p>
                    <p class="text-base font-normal w-32">Ad Type</p>
                    <span class="w-auto flex gap-3" id="inputFieldsNameContainer_${FlightId}_${CampaignId}_${CampaignLan}_${CampaignType}_${CategoryMasterId}_${counter}"></span>
                </div>
                <div class="z-10 mr-3">
                    <p class="text-lg w-[165px] text-start">Due to Publisher</p>
                </div>
            </div>
            <div class="w-full flex justify-between">
    `;
    ...
    if (CategoryName != 'social') {
            ...
            .then(data => {
                ...
                htmlContent += `
                    <div class="w-full flex gap-3">
                        <input type="text" value="" name="NewAdPublisher[${FlightId}][${CampaignId}][${CampaignLan}][${CampaignType}][${CategoryMasterId}][]" class="input input-bordered input-primary w-32 rounded-md" placeholder="publisher">
                        <select id="AdType_${counter}" onchange="getInputFields(${FlightId},${CampaignId},'${CampaignLan}','${CampaignType}',${CategoryMasterId},'${CategoryName}',${counter})" class="select select-primary w-32 rounded-md" name="NewAdType[${FlightId}][${CampaignId}][${CampaignLan}][${CampaignType}][${CategoryMasterId}][]">
                            ${optionsHTML}
                        </select>
                        <span class="w-auto flex gap-3" id="inputFieldsContainer_${FlightId}_${CampaignId}_${CampaignLan}_${CampaignType}_${CategoryMasterId}_${counter}"></span>
                    </div>
                    <div class="z-10 mr-3">
                        <input type="date" value="" name="NewAdDate[${FlightId}][${CampaignId}][${CampaignLan}][${CampaignType}][${CategoryMasterId}][]" class="input input-bordered input-primary w-[165px]" placeholder="MM/DD/YYYY" id="DueToPublisher">
                    </div>
                </div>
                <div class="divider my-0"></div>
            </div>`;
                ...
                document.querySelector('#AddContainer_' + CampaignId).insertAdjacentHTML('beforeend', htmlContent);
            })
            .catch(error => {
        ...
        fetch('/fetch-advertisement-types/' + CategoryMasterId)
            ...
            .then(data => {
                ...
                htmlContent += `
                    <div class="w-full flex gap-3">
                        <select id="publisher_${CategoryMasterId}_${counter}" onchange="getPublisherAdType(${CategoryMasterId},${counter})" class="select select-primary w-32 rounded-md" name="NewAdPublisher[${FlightId}][${CampaignId}][${CampaignLan}][${CampaignType}][${CategoryMasterId}][]">
        ...
        // Append the completed htmlContent to the container for default case
        document.querySelector('#AddContainer_' + CampaignId).innerHTML += htmlContent;
    }
}
    ...
    //                             <input required type="date" value="" name="AdDate[` + FlightId +
        ...
        //                     `][` + CampaignType + `][` + CategoryMasterId + `][]" class="input input-bordered input-primary w-[165px]" placeholder="MM/DD/YYYY" ="" id="DueToPublisher">
    //                         </div>
    //                     </div>
    //                     <div class="divider my-0" =""></div>
    //                     </div>`;
    ...
    <script>
        function getPublisherAdType(id, counterId) {
            var publisher_Id = document.getElementById('publisher_' + id + '_' + counterId).value;
            $.ajax({
                type: 'POST',
                dataType: 'json',
                ...
                success: function(response) {
                    $('#AdType_' + counterId).empty();
                    $('#AdType_' + counterId).append(
